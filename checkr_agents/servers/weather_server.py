...
from checkr_agents.agents.weather_nlip_agent import WeatherNlipAgent
...
class WeatherManager(SessionManager):
    ...
    async def process_nlip(self, msg: NLIP_Message) -> NLIP_Message:
        ...
        try:
            ...
            logger.info(f"WeatherServerResults : {results}")
...
# app = server.app
app = NlipSessionServer("WeatherCookie", WeatherManager)

if __name__ == "__main__":

    parser = argparse.ArgumentParser("Run the WeatherServer on a specified port (default 8022)")
    parser.add_argument("--port", type=int, default=8022, help="the network port to serve on")
    ...
    if args.uds == None:
        uvicorn.run("checkr_agents.servers.weather_server:app", host="0.0.0.0", port=args.port, log_level="info", reload=True)
    else:
        uvicorn.run("checkr_agents.servers.weather_server:app", uds=args.uds, log_level="info", reload=True)
